...
import getTranslatedText from './TranslatedText'; 
...
import LargeProtozoa from '@/components/icons/LargeProtozoa'
import Mite from '@/components/icons/Mite'
import TranslateTextComponent from './TranslatedText';
...
const SoilFoodWebFrenzy: React.FC = () => {
  ...
  const [language, setLanguage] = useState<'en' | 'es'>('en');  const gameAreaRef = useRef<HTMLDivElement>(null);
 ...
 const [touchStart, setTouchStart] = useState<{ x: number; y: number } | null>(null)
  ...
  const handleTouchEnd = useCallback((e: React.TouchEvent) => {
    if (!touchStart || !gameAreaRef.current) return;
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY)) {
      // Horizontal swipe
      if (deltaX > 0) {
        setProtozoa(prev => moveCharacter(prev, 'ArrowRight'));
      } else {
        setProtozoa(prev => moveCharacter(prev, 'ArrowLeft'));
      }
    } else {
      // Vertical swipe
      if (deltaY > 0) {
        setProtozoa(prev => moveCharacter(prev, 'ArrowDown'));
      } else {
        setProtozoa(prev => moveCharacter(prev, 'ArrowUp'));
      }
    }
    setTouchStart(null);
  }, [touchStart, moveCharacter]);
...
const renderGrid = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white p-4">
      ...
      <h1 className="text-4xl font-bold mb-4"><TranslateTextComponent textKey="soilFoodWebFrenzy" language={language} /></h1>
      <Button onClick={toggleLanguage} className="mb-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
        {language === 'en' ? 'Español' : 'English'}
      </Button>
      <div className="mb-4 text-lg">
        <span className="mr-4"><TranslateTextComponent textKey="level" language={language} />: {level + 1}</span>
        <span className="mr-4"><TranslateTextComponent textKey="score" language={language} />: {score}</span>
        <span><TranslateTextComponent textKey="lives" language={language} />: {lives}</span>
      </div>

        <div>
          <h2 className="text-lg font-bold mb-0"><TranslateTextComponent textKey="howToPlay" language={language} /></h2>
          <div className='flex'>
            <p><TranslateTextComponent textKey="useArrowKeys" language={language} />
            </p>
            <figure className='ml-4 pt-1 w-9'>
              <Protozoa></Protozoa>
            </figure>
          </div>

            <p className='pb-0'><TranslateTextComponent textKey="avoidEnemies" language={language} /></p>
            <ul className="list-disc list-inside flex ">
              <li className='mr-8 font-bold text-sm'><TranslateTextComponent textKey="nematode" language={language} />:
              <figure className='w-20 mx-auto'>
              <ParasiteNematode></ParasiteNematode>
              </figure>
              </li>
              <li className='mr-8 font-bold text-sm'><TranslateTextComponent textKey="predatoryMite" language={language} />:
              <figure className='w-14 pt-2 mx-auto'>
              <Mite></Mite>
              </figure>
              </li>
              <li className='mr-8 font-bold text-sm'><TranslateTextComponent textKey="largerProtozoa" language={language} />:
              <figure className='w-16 pt-2 mx-auto'>
              <LargeProtozoa></LargeProtozoa>
              </figure>
              </li>
            </ul>
        </div>
      ...
      <div
        ...
        style={{
          width: isMobile ? '100%' : `${GRID_SIZE * CELL_SIZE}px`,
          height: isMobile ? 'auto' : `${GRID_SIZE * CELL_SIZE}px`,
      ...
      >
        {isGameStarted ? renderGrid() : (
          <div className="flex items-center justify-center h-full">
            ...
            >
              <TranslateTextComponent textKey="startGame" language={language} /> <Play className="ml-2" size={16} />
            </Button>
          </div>
        )}
      </div>
      {levelComplete && (
        ...
        >
          <TranslateTextComponent textKey="nextLevel" language={language} /> <ArrowRight className="ml-2" />
        </Button>
      )}
      {gameOver && (
        <div className="mt-4 text-2xl font-bold text-red-500"><TranslateTextComponent textKey="gameOver" language={language} /></div>
      )}
      {infoMessage && (
      ...
      <div className="mt-4 text-sm text-center max-w-4xl grid grid-cols-2 gap-4">
        ...
        <div>
          <h2 className="text-lg font-bold mb-2"><TranslateTextComponent textKey="powerUps" language={language} /></h2>
          <ul className="list-disc list-inside">
            <li>F: <TranslateTextComponent textKey="fungalHyphae" language={language} /></li>
            <li>O: <TranslateTextComponent textKey="organicMatter" language={language} /></li>
            <li>D: <TranslateTextComponent textKey="compost" language={language} /></li>
            <li>N: <TranslateTextComponent textKey="biochar" language={language} /></li>
            <li>R: <TranslateTextComponent textKey="humus" language={language} /></li>
          </ul>
          <p><TranslateTextComponent textKey="collectInfoBubbles" language={language} /></p>
        </div>
      </div>
      <Dialog open={isPaused} onOpenChange={setIsPaused}>
        <DialogContent className="sm:max-w-[425px] bg-gray-800 text-white">
          <DialogTitle><TranslateTextComponent textKey="soilFoodWebFact" language={language} /></DialogTitle>
          <DialogDescription>
          ...
          >
            <TranslateTextComponent textKey="close" language={language} />
          </Button>
        </DialogContent>
      </Dialog>
    </div>
